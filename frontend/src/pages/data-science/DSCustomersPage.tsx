...
import { Users, Heart, Target, Repeat } from 'lucide-react';
  ...
  <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-5 border-b border-slate-100 bg-slate-50"><h3 className="font-semibold text-slate-800">Segment Profiles</h3></div>
        <table className="w-full text-sm text-left text-slate-600">
          <thead className="text-xs text-slate-500 uppercase bg-slate-100 border-b border-slate-200">
            <tr><th className="px-4 py-2.5">Segment</th><th className="px-4 py-2.5 text-right">Avg Policies</th><th className="px-4 py-2.5 text-right">LTV</th><th className="px-4 py-2.5 text-center">Churn Risk</th></tr>
          </thead>
          <tbody>
            {segments.map(s => (
              <tr key={s.name} className="border-b border-slate-100 hover:bg-slate-50">
                ...
                <td className="px-4 py-2.5 text-right font-mono">{s.avgPolicies}</td>
                <td className="px-4 py-2.5 text-right font-mono">{s.ltv}</td>
